...
struct OnboardingGoalsView: View {
    ...
    init(scale: CGFloat, userPromptText: String, animate: Bool, initialSelected: Set<String> = [], onContinue: @escaping (Set<String>) -> Void) {
        self.scale = scale
    ...
    private func animateIn() async {
        guard animate else {
            showPrompt = true
            typed1 = L10n.Onboarding.goalsResponse1
            typed2 = L10n.Onboarding.goalsResponse2
            showOptions = true
            return
        }
        ...
        await TypewriterAnimation.typeOut(L10n.Onboarding.goalsResponse1, reduceMotion: reduceMotion) { typed1 = $0 }
        try? await Task.sleep(for: .milliseconds(200))
        await TypewriterAnimation.typeOut(L10n.Onboarding.goalsResponse2, reduceMotion: reduceMotion) { typed2 = $0 }
...
#Preview {
    OnboardingGoalsView(
        scale: 1,
        userPromptText: "Not sure I'm saving enough",
        animate: true,
        onContinue: { _ in }
    )
