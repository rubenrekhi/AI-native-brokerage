...
│   ├── account_activity.py         # GetAccountActivity — unified transaction-history feed (pill)
│   └── transfer_operations.py      # TransferOperations — propose deposit/withdraw (HIL; no pill); see tools/transfer_operations.md
├── transport/
│   ├── __init__.py                 # empty
...
Most tools have a dedicated reference under [`tools/`](tools/) — [`get_stock_info`](tools/get_stock_info.md), [`display_stock_card`](tools/display_stock_card.md), [`radar_operations`](tools/radar_operations.md), [`get_portfolio`](tools/get_portfolio.md), [`get_portfolio_performance`](tools/get_portfolio_performance.md) (these two share `utils/portfolio_tool_runtime.py`), [`transfer_operations`](tools/transfer_operations.md) (the first human-in-the-loop tool — [`hil-actions.md`](hil-actions.md)); `get_account_activity` is summarized only in the rows below. The rows below are the harness-level summary.
...
| `__init__.py` · `build_default_registry()` / `DEFAULT_REGISTRY` | Registers `GetStockInfo` + `DisplayStockCard` + `GetPortfolio` + `GetPortfolioPerformance` + `RadarOperations` + `GetAccountActivity` + `TransferOperations`. | Module-level singleton used by `post_turn`. |
...
| `account_activity.py` · `GetAccountActivity` | Read the user's own transaction history — a unified, newest-first feed of executed/working trades, deposits/withdrawals, dividends, interest, with per-type `totals` (incl. `executed_trades` vs `open_orders` counts). Optional `activity_types` / `after` / `until` / `symbol` / `include_canceled` / `limit` filters. Emits a "Looking at your account activity" pill. | Data goes to `model_payload`, not a card. `partial: true` when one source was unavailable; expected failures return `{"error"}` and never raise. Reuses `ActivityService`; no new wire `Block`. (No dedicated `tools/` doc yet.) |
| `transfer_operations.py` · `TransferOperations` | **Propose** a deposit (`INCOMING`) or withdrawal (`OUTGOING`) between the user's linked bank and Sevino — the first **human-in-the-loop** tool. Validates `amount`, resolves the source bank (asks via `needs_clarification` when several apply), and **moves no money**: returns a `ConfirmationBlock(kind="transfer")` + `ProposedAction(action_type="transfer")`. | The proposal raises the HIL gate → turn ends `awaiting_confirmation`; the ACH transfer runs in `TransferActionHandler` (`actions/transfer.py`) only after the user taps Confirm. Emits **no pill** (the card is the `ui_block`). Errors: `NO_LINKED_BANK` / `BANK_NOT_APPROVED` / `BROKERAGE_UNAVAILABLE`. Full detail: [`tools/transfer_operations.md`](tools/transfer_operations.md); framework: [`hil-actions.md`](hil-actions.md). |
